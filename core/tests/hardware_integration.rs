...
use sayonara_wipe::{WipeConfig, Algorithm, WipeOrchestrator};
...
async fn test_wipe_nvme_advanced() -> Result<()> {
    ...
    let mock = MockNVMeDriveBuilder::new()
...
async fn test_verification_after_wipe() -> Result<()> {
    ...
    assert!(result.success_rate > 0.99, "Verification should show high success rate");
    assert!(result.mismatches == 0, "Should have no mismatches for zero pattern");
